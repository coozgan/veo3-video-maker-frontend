...
function KlingBadge() {
    return (_jsxs("span", { className: "inline-flex items-center gap-1 rounded-full bg-accent-soft px-2 py-0.5 text-xs font-medium text-accent", children: [_jsx(Sparkles, { size: 11 }), "via Kling 3.0"] }));
}
export function JobStatus({ operationName, result, error }) {
    ...
    if (!result || result.state === "running") {
        const onKling = result?.provider === "kling";
        const isFailover = result?.failover === true;
        return (_jsxs(Card, { className: "flex flex-col gap-3 p-4", children: [_jsxs("div", { className: "flex items-center gap-2 text-sm", children: [_jsx(Loader2, { size: 16, className: "animate-spin text-accent" }), _jsx("span", { className: "font-medium text-fg", children: "Generating\u2026" }), _jsxs("span", { className: "text-fg-muted", children: [elapsed, "s elapsed"] }), onKling && _jsx(KlingBadge, {})] }), _jsx("div", { className: "bar-indeterminate h-1.5 w-full rounded-full" }), _jsx("p", { className: "text-xs text-fg-subtle", children: isFailover
                        ? "Veo blocked the content — retrying with Kling 3.0. This may take a moment."
                        : onKling
                            ? "Generating with Kling 3.0 — this may take a moment."
                            : "Veo typically takes 30 seconds to a few minutes per clip. You can leave this tab open." })] }));
    }
    return (_jsxs(Card, { className: "overflow-hidden rounded-xl p-0", children: [_jsx("video", { src: result.videoUrl, controls: true, autoPlay: true, loop: true, className: "block w-full bg-black" }), _jsxs("div", { className: "flex items-center justify-between gap-3 border-t border-border px-4 py-4", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-sm text-fg-muted", children: "Your video is ready." }), result.provider === "kling" && _jsx(KlingBadge, {})] }), _jsx("a", { href: result.videoUrl, download: true, children: _jsxs(Button, { variant: "outline", size: "sm", type: "button", children: [_jsx(Download, { size: 14 }), "Download"] }) })] })] }));
}

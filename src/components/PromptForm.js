...
export function PromptForm({ onSubmit, disabled }) {
    ...
    async function handleSubmit(e) {
        ...
        if (!prompt.trim())
            return;
        const base = { prompt: prompt.trim(), mode, tier, duration, aspectRatio, generateAudio };
        try {
            if (mode === "image") {
                if (!image.file) {
